...
namespace System {
	...
	public:
		...
		static bool TryGetNamedColorToColor32(const std::string str, Color32& color) {
	...
	{ "yellowgreen", { 154, 205, 50, 255 } }
		...
		static bool TryGetColor32(const std::string str, Color32& color) {
			...
			if (*p == '#') {
				...
				for (int i = 0; i < (isRgba ? 4 : 3); i++) {
					while (p < e && std::isspace(static_cast<unsigned char>(*p))) ++p;
					if (p >= e) return false;

					const char* num_start = p;
					while (p < e && (std::isdigit(static_cast<unsigned char>(*p)) || *p == '.')) ++p;
					std::string num_str(num_start, p - num_start);
					if (num_str.empty()) return false;

					if (i < 3) { // R, G, B
						components[i] = std::stoi(num_str);
					}
					else { // Alpha
						double alpha = std::stod(num_str);
						if (alpha <= 1.0) components[i] = static_cast<int>(alpha * 255);
						else components[i] = static_cast<int>(alpha);
					}
				...
				color.r = std::max<int>(0, std::min<int>(255, components[0]));
				color.g = std::max<int>(0, std::min<int>(255, components[1]));
				color.b = std::max<int>(0, std::min<int>(255, components[2]));
				color.a = std::max<int>(0, std::min<int>(255, components[3]));
				return true;
			}else{
				if (TryGetNamedColorToColor32(str, color)) {
					return true;
				}
			}
		}
